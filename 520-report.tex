...
particular subsection of a network. NetMapJs uses semantic zooming along with
panning to effectively navigate throughout large networks. 
...
point, the group node graphic and its external edges become transparent which
...
%   > Edges
%     * Both thought edges were effective
%     * Rural Link would like to see live data
%     * Why do I think live data would be a good idea?
%       - movement of arrows over time, would it be distracting etc
%       - 
%   > Nodes
%     * Groups are good at showing distant sub-network detail
%     * At a high level view can see which nodes/pops have the most connected
%     sites
%     * Work well at hiding detail to simplify large maps also
%     * Navigation combined with overviews useful
%
%   > VLANs
%     * Both liked being able to view VLANs separately
%     * Discussion?
%       - Shows that the overlay system is very useful
%       - Other tools haven't separated out VLAN paths
%       - Useful for visualising shape of VLANs.
%       - Could separate out traffic is data available...
...
drawing the edge graphics and as described earlier, NetMapJs supports polling this
data and updating the map in real time. Live data updating the visualisation may
raise other issues such as whether consistent large changes in bandwidth usage
would distract users from other parts of the map. 

The networks identified nodes as being effective at showing distant sub-network
detail while still hiding information. Showing some limited detail of deeper
sub-network levels indicates that the nodes exist and show at a higher level
what group nodes have the most connected devices. The hiding of detail is
effective at simplifying large network maps. Rural Link identified areas of the
network map that could be converted to groups. Automatically deciding on what
nodes to assign to a group can be difficult to get right and so it would be
useful for NetMapJs to include an easy way of manually building groups.
Selecting a set of nodes through user interaction in the map editor and then
assigning that selection to a group would be a good way of implementing this.
...
over top of the normal edge graphics. There is much more potential for this
...
% What they both didn't like
%   > Aspects of the UI
%     * Not immediately clear of some features - like follow paths
%     * Need to add more visual cues such as...
%     * Need to be able to customise with own colours, fonts, branding etc
%       - how have I developed to easily support own colours schemes.
%     * Zooming - should have limits and should center around mouse position
%       - expand, expand, expand
%     * Resize window?
%
%
% Evaluation of how NetMapJs compares with both other maps
%   > ...
%
%
% Other comments?
%   > Karen - provoked thought in their team
%   > Rural Link - geographical, logical or automatic.
%
%
% Both Networks would consider using NetMapJs once polished
%   * What does this show?
%   * NetMapJs is an improvement over their existing network maps
...
some features more obvious and the styles used make aspects of the map hard to
see. The visual cue, a tooltip, for following edges to the node on the other end
only appear when the user hovers the mouse over the edge. A graphic that
appears when following an edge without the user having to provide interaction
would make this feature much more obvious. 

Some of the colour choices on the map designs such as red on black make parts of
the map hard to see. While more of the focus in this project has been placed on
the development of new network map display techniques, it is still important to
make the design visually appealing. Users are not going to care about the
advanced network map functionality if it does not look good. In NetMapJs all of
the styles, colours, fonts, node and edge graphics and user interface layout are
customisable. The visual styles were separated from the implementation to make
it trivial for developers to customise the look of the map to suit the networks
requirements.
...
limits imposed to stop them going to far in either direction. This would support
the user to get an idea of the size of the network without having to guess based
on the fact that no more detail is being added from zooming in or there are no
more nodes off the screen when zooming out. The zooming out limit could be based
on the minimum and maximum coordinates of the furtherest nodes in each direction
on the screen. Zooming in could be restricted to the deepest sub-network in the
map.
...
view port around that. This way, the panning and zooming are easily achieved
together using the mouse pointer position.

A comparison of NetMapJs to the networks existing network maps found it to be
much more interactive, even more so than the new Icinga network management
...
visualisation of networking performance data are also improvements of NetMapJs
...
NetMapJs which allows for improvements such as easy overlay integration,
semantic zooming and live update support will make it an improvement over this
tool when the issues with the user interface have been addressed.

Both of the networks that evaluated NetMapJs would consider using it as a
replacement to their current network map and mentioned that it would make a good
upgrade. KAREN noted that NetMapJs had provoked some thought in their team.
Overall, this feedback shows that this project had been effective at developing
improvements to computer network maps.
